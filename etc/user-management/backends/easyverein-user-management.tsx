...
import type { User, UserManagement } from '../types'
...
export class EasyVereinUserManagement implements UserManagement {
  private client = new EasyVereinClient(
    process.env.EASYVEREIN_API_KEY as string
  )
  ...
  async getUser(email: string): Promise<User | null> {
    ...
    if (members.length > 0) {
      const contactDetails = await this.client.getContactDetails(
